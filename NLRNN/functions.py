...
# for NLRNN
class Dataset_NLRNN(data.Dataset):
    "Characterizes a dataset for PyTorch"
    def __init__(self, features, folders, labels, frames):
        "Initialization"
        self.data = features
        self.labels = labels
    ...
    def load_NL_feats(self, selected_folder):
        X, y = [], []
 
        print(selected_folder)
        frame_names = list(self.data[selected_folder].keys())
        frame_names.sort()   # frames of video sorted

        for i in range(len(self.frames)-1):
            frame_i = frame_names[i]
            frame_j = frame_names[i+1]
           
            feat1 = self.data[selected_folder][frame_i][0]
            feat2 = self.data[selected_folder][frame_j][0]

            feat = torch.cat((feat1.squeeze(), feat2.squeeze()), 0)
            X.append(feat)
            y.append(self.data[selected_folder][frame_j][1])

        X = torch.stack(X, dim=0) 
        y = torch.stack(y, dim=0) 
        # y = torch.LongTensor(y)
        # print('X: ',X.size()) #torch.Size([48, 4096])
        # print('y: ',y.size()) #torch.Size([48])
        return X, y
    ...
    def __getitem__(self, index):
        ...
        # Load data
        X,y = self.load_NL_feats(folder)     # (input) spatial images
        # y = torch.LongTensor([self.labels[index]])    # (labels) LongTensor are for int64 instead of FloatTensor
...
class DecoderRNN(nn.Module):
    def __init__(self, CNN_embed_dim=1024, h_RNN_layers=1, h_RNN=256, h_FC_dim=128, drop_p=0.3, num_classes=6):
        ...
        self.lstm = nn.LSTMCell(input_size=self.RNN_input_size, hidden_size=self.h_RNN, bias=True)

        # TODO: change dims of FC layers
        # self.fc1 = nn.Linear(self.h_RNN, self.h_FC_dim)
        # self.fc2 = nn.Linear(self.h_FC_dim, self.num_classes)
        

        self.fc3 = nn.Linear(self.h_RNN, self.h_FC_dim)
    ...
    def forward(self, x_gaze, h0, c0):
        ...
        x = self.fc1(x_gaze[:,0,:])   # features for first frame
        x = F.relu(x)
        # x = F.dropout(x, p=self.drop_p, training=self.training)
        x = self.fc2(x)
        ...
        # take RNN output at all time steps
        # x = RNN_out

        return out  # output at each of the 50 time steps
